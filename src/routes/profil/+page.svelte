	...
	<div class="p-5">
		...
		>
			...
			<button
				...
				class="w-full rounded-xl bg-success-600 px-4 py-3 font-bold text-white transition-all hover:bg-success-700 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
			>
				{savingIncomes ? 'Speichert...' : 'Einkommen speichern'}
			</button>
		</form>
	</div>
</div>
	...
	<div class="p-5">
		...
		>
			...
			<button
				...
				class="w-full rounded-xl bg-accent-600 px-4 py-3 font-bold text-white transition-all hover:bg-accent-700 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
			>
				{savingPrepayment ? 'Speichert...' : 'Vorauszahlung speichern'}
			</button>
		</form>
	</div>
</div>
...
<div class="mb-6">
	<button
		...
		class="mb-3 flex w-full items-center justify-between rounded-xl border-2 border-neutral-200 bg-white px-5 py-4 font-bold text-neutral-900 transition-all hover:bg-neutral-50 active:scale-95"
	>
		<span class="flex items-center gap-2">
			<svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
			</svg>
			Archiv ({data.closedMonths.length})
		</span>
		<svg class="h-5 w-5 transition-transform {showArchive ? 'rotate-180' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
	...
	{#if showArchive}
		<div class="space-y-2">
			{#each data.closedMonths as closedMonth}
				<div class="flex items-center justify-between rounded-lg border-2 border-neutral-200 bg-neutral-50 px-4 py-3">
					<div>
						<p class="font-semibold text-neutral-900">
							{formatMonthYear(closedMonth.year, closedMonth.month)}
						</p>
						<p class="text-sm text-neutral-600">
							Endsaldo: {formatEuro(closedMonth.private_balance_end || 0)}
						</p>
					</div>
					<form
						method="POST"
					...
					>
						...
						<button
							type="submit"
							class="rounded-lg border-2 border-danger-200 bg-danger-50 px-3 py-2 text-sm font-semibold text-danger-700 transition-all hover:bg-danger-100 active:scale-95"
							onclick={() => confirm(`Monat ${formatMonthYear(closedMonth.year, closedMonth.month)} wirklich löschen?`)}
						>
							Löschen
						</button>
					</form>
			...
			{:else}
				<p class="text-center text-sm text-neutral-500">Keine archivierten Monate vorhanden</p>
			{/each}
		</div>
	{/if}
</div>
