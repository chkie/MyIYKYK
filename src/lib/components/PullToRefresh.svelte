...
>
	...
	{#if (isPulling && pullDistance > 10) || isRefreshing}
		...
		>
			<div class="bg-primary-100 flex h-10 w-10 items-center justify-center rounded-full shadow-lg">
				{#if isRefreshing}
					<svg class="text-primary-600 h-6 w-6 animate-spin" fill="none" viewBox="0 0 24 24">
				...
				{:else}
					<svg
						class="text-primary-600 h-6 w-6 transition-transform"
	...
	<div
		style="transform: translateY({isPulling || isRefreshing
			...
			: 0}px); transition: {isPulling ? 'none' : 'transform 0.3s ease-out'}"
	>
		{@render children()}
	</div>
</div>
