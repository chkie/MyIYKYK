<script lang="ts">
	...
	let swipeDistance = $derived(
		isSwiping ? (isLocked ? currentX - startX + lockedOffset : currentX - startX) : lockedOffset
	);
...
<div class="relative overflow-hidden bg-white">
	...
	<div class="absolute inset-y-0 right-0 z-0 flex items-center justify-end gap-2 pr-2">
		{#if onEdit}
			<button
				type="button"
				...
				class="bg-primary-500 flex h-12 w-12 items-center justify-center rounded-xl text-white shadow-lg transition-all active:scale-90"
			>
				<svg
					class="h-5 w-5"
					fill="none"
		...
		{#if onDelete}
			<button
				...
				class="bg-danger-500 flex h-12 w-12 items-center justify-center rounded-xl text-white shadow-lg transition-all active:scale-90"
			>
				<svg
					class="h-5 w-5"
					fill="none"
